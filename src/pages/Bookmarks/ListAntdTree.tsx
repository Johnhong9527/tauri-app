...
export default function List() {
  ...
  useEffect(() => {
    if (treeData.length) {
      // mindInit();
      setTreeDataList(getTreeData(treeData));
    }
  }, [treeData]);
  ...
  function getTreeData(useTreeData: any) {
    // TODO 从数据中读取到了数据
    const treeObj = [] as any;
  ...
  const onRightClick = ({ event, node }: any) => {
    console.log(446, event, node);
    setNodeContextMenuShow(true);
    setNodeContextMenuLeft(event.clientX + 10);
    setNodeContextMenuTop(event.clientY + 10);
  };
  return (
    <div className={styles.ListPage}>
      ...
      {/* 移动到别的目录下 */}
      {/* <div className={styles.dirTreeList}>dirTreeList</div> */}
      <Row>
        <Space>
          {/*导入文件*/}
